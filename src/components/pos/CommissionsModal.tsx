...
import { Percent, X, Eye, ChevronDown, CalendarDays, AlertTriangle } from 'lucide-react'
...
const MOCK_MONTHLY_COMMISSIONS: Record<string, CommissionRow[]> = {
  ...
  '2026-07': [
    { id: '1', name: 'Jessica Roy', serviceRevenue: 38500, commissionRate: 0.2, status: 'Paid' },
    { id: '2', name: 'David Kumar', serviceRevenue: 31200, commissionRate: 0.25, status: 'Paid' },
    { id: '3', name: 'Mia Thomas', serviceRevenue: 14200, commissionRate: 0.2, status: 'Pending' },
  ],
  '2026-06': [
    { id: '1', name: 'Jessica Roy', serviceRevenue: 40100, commissionRate: 0.2, status: 'Paid' },
    { id: '2', name: 'David Kumar', serviceRevenue: 26900, commissionRate: 0.25, status: 'Pending' },
    { id: '3', name: 'Mia Thomas', serviceRevenue: 16800, commissionRate: 0.2, status: 'Paid' },
  ],
}
...
export default function CommissionsModal({
  ...
  const [selectedBreakdown, setSelectedBreakdown] = useState<CommissionRow | null>(null)
  ...
  function handleConfirmProcessAll() {
    setRows((prev) =>
      prev.map((row) =>
        row.status === 'Pending'
  ...
  function handleProcessAllClick() {
    if (pendingCount === 0) {
  ...
  return (
    ...
    >
      <div className="relative flex h-full max-h-[min(800px,92dvh)] w-full max-w-[920px] flex-col overflow-hidden rounded-2xl border border-salon-border bg-white shadow-xl">
        ...
        {/* Custom In-App Confirmation Toast/Modal */}
        {showConfirmModal && (
          <div
            ...
            aria-modal="true"
          >
            <div className="w-full max-w-md overflow-hidden rounded-2xl border border-salon-border bg-white p-6 shadow-2xl">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-salon-muted">
                    Are you sure you want to process payouts for all {pendingCount} pending staff commission{pendingCount > 1 ? 's' : ''}?
                  </p>
                </div>
              </div>
              <div className="mt-6 flex justify-end gap-3">
                <button
                  ...
                  className="rounded-lg border border-salon-border px-4 py-2 text-xs font-semibold text-salon-text hover:bg-slate-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={handleConfirmProcessAll}
                  className="rounded-lg bg-[#6b1d2f] px-4 py-2 text-xs font-semibold text-white hover:bg-[#5a1828] transition-colors"
                >
                  Confirm & Process
                </button>
              </div>
